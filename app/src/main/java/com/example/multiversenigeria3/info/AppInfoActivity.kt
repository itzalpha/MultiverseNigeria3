...
import com.example.multiversenigeria3.databinding.ActivityAppInfoBinding
...
import kotlinx.android.synthetic.main.activity_app_info.view.*
...
class AppInfoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val AppUploadDate = intent.getStringExtra("AppUploadDate")
        val AppAlternativeLink = intent.getStringExtra("AppAlternativeLink")
        val AppInformation = intent.getStringExtra("AppInformation")
         docId = intent.getStringExtra("docId").toString()
        val AppPlayStoreLink = intent.getStringExtra("AppPlayStoreLink")
        val AppVerseRate = intent.getStringExtra("AppVerseRate")
        ...
        binding.infoAppTitle.text = AppName
        binding.categoryRating.text = "Category : $AppCategory \t Verse Store : $AppVerseRate"
        binding.infoAppInformation.text = AppInformation
        binding.alternativeLink.text = AppAlternativeLink
        binding.playStoreLink.text = AppPlayStoreLink
        ...
        binding.appInfoShare.setOnClickListener {
            ShareCompat.IntentBuilder.from(this)
                ...
                .setText("$AppName\n$AppInformation\n$AppPlayStoreLink get more local apps information in the app ''Verse Store''.Download App : http://play.google.com/store/apps/details?id="+applicationContext.packageName)
                .startChooser()
        }

        binding.appInfoImageLike.setOnClickListener {
            val dob = db.collection("VerseStore").document("Admin").collection("App_Likes")
            ...
            dob.get().addOnCompleteListener { task ->
                ...
                if (task.result.exists()) {
                    ...
                    val doc =
                        db.collection("VerseStore").document("Admin").collection("App_Likes")
                            .document(docId.toString()).collection("Users").document(currentUserId)
    ...
    private fun EventChangeListener() {
        ...
        db.collection("VerseStore").document("Admin").collection("Reviews").document("AppReviews")
            .collection(docId)
    ...
    private fun addReview() {
        ...
        val db = FirebaseFirestore.getInstance().collection("VerseStore").document("Admin")
            .collection("Reviews").document("AppReviews").collection(docId).document(currentUserId)
